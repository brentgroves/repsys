#!/bin/bash
# ./sed-mysql-service mysql rephub11 30031 3306 mysql
app=$1
node=$2
node_port=$3
target_port=$4
...
sed s/%APP%/$app/g templates/service-template.yaml | \
sed s/%NODE%/$node/g | \
sed s/%NODE_PORT%/$node_port/g | \
sed s/%TARGET_PORT%/$target_port/g | \
sed s/%TARGET_PORT_NAME%/$target_port_name/g > templates/service.yaml 